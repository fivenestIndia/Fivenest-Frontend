...
var PLUGIN_ID = "92c18351"; // Using active ID
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    function openWhatsAppSupport() {
        ...
        var rawMessage = "Hello Vilesh, I need assistance with FN Pro for Illustrator. (User: " + nameSection + ")";
...
async function runEngine() {
    ...
    try {
        ...
        if (doMockup) {
            ...
            var csvNameClean = selectedCSVFile.name.replace(/\.[^/.]+$/, "");
            var mockupResult = await runHostScript("generateMockups('" + selectedFolder.replace(/\\/g, "\\\\") + "', '" + csvNameClean.replace(/'/g, "\\'") + "')");
            if (mockupResult.indexOf("SUCCESS") > -1) {
