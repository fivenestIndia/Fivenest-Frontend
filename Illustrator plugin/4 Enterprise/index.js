...
var selectedCSVPath = null; // Store absolute file/folder path
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    document.getElementById("chkMultiTeam").addEventListener("change", function(e) {
        ...
        if (isMulti) {
            ...
            log("Multi-Team Mode enabled. Select folder of CSVs matching your Illustrator layout.");
        } else {
    ...
    document.getElementById("btnCSVInput").addEventListener("click", function() {
        ...
        var result = window.cep.fs.showOpenDialog(false, isMulti, isMulti ? "Select CSV Folder" : "Select CSV File", "", isMulti ? [] : ["csv", "txt"]);
        if (result && result.data && result.data.length > 0) {
            selectedCSVPath = cleanCEPPath(result.data[0]);
            var parts = selectedCSVPath.split(/[\\\/]/);
            var nodeName = parts[parts.length - 1] || parts[parts.length - 2] || "Selected";
            document.getElementById("lblCSVInput").innerText = nodeName;
            document.getElementById("lblCSVInput").style.color = "#4CAF50";
            log((isMulti ? "CSV Folder Set: " : "CSV File Set: ") + selectedCSVPath);
        }
    });
    ...
    document.getElementById("btnRefineAI").onclick = function() {
        csInterface.openURLInDefaultBrowser("https://gemini.google.com/gem/1vc3MbyzLtt5RspOpQualSpuViseurHd4?usp=sharing");
    };
    ...
    function openWhatsAppSupport() {
        ...
        var rawMessage = "Hello Vilesh, I need assistance with FN Enterprise for Illustrator. (User: " + nameSection + ")";
...
async function validateCSV() {
    ...
    if (isMulti) {
        ...
        var fs = require('fs');
        var entries = fs.readdirSync(selectedCSVPath);
        var csvFiles = entries.filter(function(e) { return e.toLowerCase().endsWith('.csv'); });
        if (csvFiles.length === 0) { log("❌ No CSV files found."); alert("No CSV files found in folder."); return; }
        log("✅ Found " + csvFiles.length + " CSV files in directory.");
    } else {
        if (!selectedCSVPath) { log("❌ Select a single CSV File first."); return; }
        log("✅ Ready to process single file: " + selectedCSVPath);
    }
}
...
async function sendToGoogleSheet(fileName, customerName, backQuantity, panelOrderNumber) {
    if (!GOOGLE_WEBAPP_URL || GOOGLE_WEBAPP_URL === "YOUR_GOOGLE_WEB_APP_URL_HERE") {
        log("⚠️ Skipping Google Sheets: URL is missing in settings.");
        return; 
    }
    try {
        log("⏳ Syncing [" + fileName + "] to Google Sheets...");
        var response = await fetch(GOOGLE_WEBAPP_URL, {
            method: "POST",
        ...
        if (resultText.indexOf("SUCCESS_SAVED") > -1) {
            log("📊 Success! Logged Order to Sheets. Qty: " + backQuantity);
        } else if (resultText.indexOf("<html") > -1 || resultText.indexOf("Sign in") > -1) {
            log("❌ GOOGLE BLOCKED IT: Set 'Who has access' to 'Anyone' in your deployment settings!");
        } else {
...
async function runEngine() {
    ...
    try {
        ...
        if (isManual) {
            ...
            var designLayersJSON = await runHostScript("scanDesignLayers('Front')");
            var designNames = JSON.parse(designLayersJSON);
            ...
            for (var dIndex = 0; dIndex < designNames.length; dIndex++) {
                ...
                if (matchedFile) {
                    var fPath = pathMod.join(selectedCSVPath, matchedFile);
                    var csvText = fs.readFileSync(fPath, 'utf8');
            ...
            var csvText = fs.readFileSync(selectedCSVPath, 'utf8');
            var csvRows = parseCSV(csvText);
            var headers = csvRows[0].map(function(h) { return h.toLowerCase().trim(); });
            var dataRows = csvRows.slice(1).filter(function(r) { return r.length > 0 && r[0] !== ""; });
            
            if (dataRows.length > 0) {
                var fileName = pathMod.basename(selectedCSVPath);
                mappedJobs.push({ design: { name: "Single_Design" }, file: { name: fileName, path: selectedCSVPath }, rows: dataRows, headers: headers });
                totalSteps = dataRows.length * (doNameNum ? 7 : 6);
            }
        }

        if (mappedJobs.length === 0) { alert("No valid CSV data rows mapped to active layers. Check your names/numbers."); return; }
        ...
        for (var jIndex = 0; jIndex < mappedJobs.length; jIndex++) {
            ...
            var designFolder = pathMod.join(selectedFolder, cleanDesignName);
            if (!fs.existsSync(designFolder)) fs.mkdirSync(designFolder);

            var frontDir = pathMod.join(designFolder, "Front");
            var backDir = pathMod.join(designFolder, "Back");
            var sleeveDir = pathMod.join(designFolder, "Sleeve");
            var nnDir = doNameNum ? pathMod.join(designFolder, "Name_Number") : null;
            ...
            // In Multi-Team mode, toggle sub-layers visibility in Front first
            if (isMultiTeam) {
                await runHostScript("setDesignVisibility('Front', '" + job.design.name + "')");
                await runHostScript("setDesignVisibility('Back', '" + job.design.name + "')");
            }
            ...
            if (chkMergeHalf) {
                ...
                for (var i = 0; i < job.rows.length; i++) {
                    ...
                    if (fs.existsSync(pathL) && fs.existsSync(pathR)) {
                        await runHostScript("mergeSleeves('" + pathL.replace(/\\/g, "\\\\") + "', '" + pathR.replace(/\\/g, "\\\\") + "', '" + pathMerged.replace(/\\/g, "\\\\") + "', '" + format + "', " + res + ", " + useLZW + ")");
                    }
                }
            }
            ...
            if (doMockup) {
                ...
                var mockupsFolder = pathMod.join(designFolder, "Mockups");
                if (!fs.existsSync(mockupsFolder)) fs.mkdirSync(mockupsFolder);
                
                await runHostScript("generateMockups('" + mockupsFolder.replace(/\\/g, "\\\\") + "', '" + actualOrderCode.replace(/'/g, "\\'") + "', true)");
                await runHostScript("generateMockups('" + mockupsFolder.replace(/\\/g, "\\\\") + "', '" + actualOrderCode.replace(/'/g, "\\'") + "', false)");
            }
