// index.js for CorelDRAW Starter Plugin
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    // Select CSV using native CorelScriptTools
    ...
    // Select Output Folder using Shell.Application
    document.getElementById("btnFolder").addEventListener("click", function() {
        try {
            var shell = new ActiveXObject("Shell.Application");
            // 0x0040 (new UI) | 0x0010 (show edit box) | 0x0001 (return only directories)
    ...
    var helpManual = [
        ...
        },
        {
            ...
            details: "When exporting to **TIFF**, check **LZW Compression** to enable LZW compression."
        },
        {
            title: "Save Default",
    ...
    function openWhatsAppSupport() {
        ...
        var rawMessage = "Hello Vilesh, I need assistance with FN Starter for CorelDRAW. (User: " + nameSection + ")";
    ...
    btnClearChat.onclick = function() {
        ...
        appendMessage("Hi! I'm your FN Starter offline assistant for CorelDRAW. Ask me about any feature or click a suggestion below!", "assistant");
    };
...
// Helper to find shape recursively in CorelDRAW DOM
...
// Find layer by name in active page
...
async function runEngine() {
    ...
    try {
        ...
        // Save current document units to restore later
        var originalUnits = doc.Unit;
        doc.Unit = 1; // Set units to Inches (cdrInch = 1)
        ...
        for (var lmIndex = 0; lmIndex < layerModes.length; lmIndex++) {
            ...
            for (var i = 0; i < dataRows.length; i++) {
                ...
                if (w > 0 && h > 0) {
                    ...
                    // Export to format
                    var filter = 774; // cdrJPEG
                    if (format === "png") filter = 802;
                    else if (format === "tiff") filter = 772;
                    
                    var comp = 0; // cdrCompressionNone
                    if (format === "tiff" && useLZW) {
                        comp = 1; // cdrCompressionLZW
                    }
