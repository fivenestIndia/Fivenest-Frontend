// index.js for CorelDRAW Enterprise Plugin

// --- 🔥 STRICT VERSIONING & SECURITY 🔥 ---
var PLUGIN_VERSION = "enterprise_v5.1";
var PLUGIN_ID = "92c18354";
var AUTH_SERVER_URL = "https://fivenest-backend.onrender.com/api/license/verify";
var GRACE_PERIOD_DAYS = 3;
var isSystemReady = false;
var PRODUCTION_LIMIT = 0; // Unlimited
var GOOGLE_WEBAPP_URL = "YOUR_GOOGLE_WEB_APP_URL_HERE"; // User sets in config
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    document.getElementById("chkManualMode").addEventListener("change", function(e) {
        ...
        if (isManual) {
            ...
            document.getElementById("btnCSVInput").disabled = true;
            ...
            document.getElementById("btnCSVInput").disabled = false;
    ...
    // Multi-Team Mode UI Change
    document.getElementById("chkMultiTeam").addEventListener("change", function(e) {
        var isMulti = e.target.checked;
        var btnCSV = document.getElementById("btnCSVInput");
        var lblCSV = document.getElementById("lblCSVInput");
        
        if (isMulti) {
            btnCSV.innerText = "Select CSV Folder";
            lblCSV.innerText = "None";
            selectedCSVPath = "";
            log("Multi-Team Mode enabled. Select folder of CSVs matching your CorelDRAW layout.");
        } else {
            btnCSV.innerText = "Select CSV File";
            lblCSV.innerText = "None";
            selectedCSVPath = "";
            log("Single CSV Mode enabled.");
        }
    });

    // Select CSV
    document.getElementById("btnCSVInput").addEventListener("click", function() {
        var isMulti = document.getElementById("chkMultiTeam").checked;
        try {
            if (isMulti) {
                var shell = new ActiveXObject("Shell.Application");
                var folder = shell.BrowseForFolder(0, "Select CSV Folder", 0x0040 | 0x0010 | 0x0001, 17);
                if (folder) {
                    selectedCSVPath = folder.Self.Path;
                    var parts = selectedCSVPath.split(/[\\\/]/);
                    var nodeName = parts[parts.length - 1] || parts[parts.length - 2] || "Folder";
                    document.getElementById("lblCSVInput").innerText = nodeName;
                    document.getElementById("lblCSVInput").style.color = "#4CAF50";
                    log("CSV Folder Set: " + selectedCSVPath);
                }
            } else {
                ...
                if (path) {
                    ...
                    document.getElementById("lblCSVInput").innerText = fName;
                    document.getElementById("lblCSVInput").style.color = "#4CAF50";
                    log("CSV File Set: " + path);
                }
            }
        } catch(e) {
    ...
    // AI Refiner smart CSV
    document.getElementById("btnRefineAI").onclick = function() {
        try {
            var sh = new ActiveXObject("WScript.Shell");
            sh.Run("https://gemini.google.com/gem/1vc3MbyzLtt5RspOpQualSpuViseurHd4?usp=sharing");
        } catch(e) {
            log("Failed to open Refine URL: " + e.message);
        }
    };
    ...
    var helpManual = [
        ...
        },
        {
            ...
            details: "Clicking **AI Smart CSV** opens a custom Gemini Gem link. You can paste messy client emails or order data there, and Gemini will instantly clean and format it into a valid production CSV file."
        },
        {
            title: "Multi-Team Mode",
            keywords: ["multi-team", "multi", "team", "folder", "all csv"],
            summary: "Multi-Team Mode allows batch-processing a folder of CSV files.",
            details: "When **Multi-Team** is checked, the plugin will ask for a folder of CSV files. It scans the CorelDRAW document's sublayers under 'Front' and matching files will be exported automatically to sub-folders."
        },
        {
    ...
    function openWhatsAppSupport() {
        ...
        var rawMessage = "Hello Vilesh, I need assistance with FN Enterprise for CorelDRAW. (User: " + nameSection + ")";
    ...
    function renderSuggestions() {
        ...
        var suggestionTitles = ["Blank Kit", "Half-Sleeve Merge", "Smart Mockup", "Quick Size Entry", "AI Smart CSV", "How to Run"];
    ...
    btnClearChat.onclick = function() {
        ...
        appendMessage("Hi! I'm your FN Enterprise offline assistant for CorelDRAW. Ask me about any feature (e.g. Blank Kit, Half-Sleeve Merge, Quick Size Entry) or click a suggestion below!", "assistant");
    };
...
function saveDefaults() {
    ...
    localStorage.setItem("fivenest_config_enterprise", JSON.stringify(config));
    log("✅ Settings Saved as Default.");
}

function loadDefaults() {
    try {
        var str = localStorage.getItem("fivenest_config_enterprise");
        if (str) {
            ...
            if(config.multi !== undefined) {
                document.getElementById("chkMultiTeam").checked = config.multi;
                document.getElementById("chkMultiTeam").dispatchEvent(new Event('change'));
            }
...
async function validateCSV() {
    ...
    var isMulti = document.getElementById("chkMultiTeam").checked;
    if (isMulti) {
        if (!selectedCSVPath) { log("❌ Select a CSV Folder first."); return; }
        try {
            var fso = new ActiveXObject("Scripting.FileSystemObject");
            var folder = fso.GetFolder(selectedCSVPath);
            var fc = new Enumerator(folder.Files);
            var csvCount = 0;
            for (; !fc.atEnd(); fc.moveNext()) {
                var fName = fc.item().Name;
                if (fName.toLowerCase().indexOf(".csv") > -1) csvCount++;
            }
            if (csvCount === 0) {
                log("❌ No CSV files found.");
                alert("No CSV files found in folder.");
            } else {
                log("✅ Found " + csvCount + " CSV files in folder.");
                alert("Folder scanned successfully!");
            }
        } catch(e) {
            log("❌ Folder Scan Error: " + e.message);
        }
    } else {
...
function scanDesignLayers(parentLayerName) {
    try {
        ...
        for (var i = 1; i <= layers.Count; i++) {
            if (layers.Item(i).Name.toLowerCase() === parentLayerName.toLowerCase()) {
                frontLayer = layers.Item(i);
                break;
            }
        }
        ...
        // Check sub-layers
        try {
            var subLayers = frontLayer.Layers;
            for (var idx = 1; idx <= subLayers.Count; idx++) {
                var name = subLayers.Item(idx).Name;
                if (name.toLowerCase().indexOf("background") === -1 && 
                    name.toLowerCase().indexOf("bg") === -1 && 
                    name.toLowerCase().indexOf("color fill") === -1) {
                    list.push(name);
                }
            }
        } catch(e) {}
        
        // Check shapes if no sub-layers found
        if (list.length === 0) {
            try {
                var shapes = frontLayer.Shapes;
                for (var idx = 1; idx <= shapes.Count; idx++) {
                    var name = shapes.Item(idx).Name;
                    if (name && name.toLowerCase().indexOf("background") === -1 && 
                        name.toLowerCase().indexOf("bg") === -1) {
                        list.push(name);
                    }
                }
...
function setDesignVisibility(parentLayerName, targetDesignName) {
    try {
        ...
        if (!frontLayer) return "NOT_FOUND";
...
async function sendToGoogleSheet(fileName, customerName, backQuantity, panelOrderNumber) {
    if (!GOOGLE_WEBAPP_URL || GOOGLE_WEBAPP_URL === "YOUR_GOOGLE_WEB_APP_URL_HERE") {
        log("⚠️ Skipping Google Sheets Sync: Setup your sheet URL first.");
        return; 
    }
    try {
        log("⏳ Syncing [" + fileName + "] to Google Sheets...");
        var payload = { fileName: fileName, customerName: customerName, quantity: backQuantity, panelOrderNumber: panelOrderNumber };
        
        // Since we are running in an ActiveX environment, standard fetch might fail or be blocked.
        // We will use sendPostRequest or standard fetch fallback.
        var data = await sendPostRequest(GOOGLE_WEBAPP_URL, payload);
...
async function runEngine() {
    ...
    var isManual = document.getElementById("chkManualMode").checked;
    var isMultiTeam = document.getElementById("chkMultiTeam").checked;
    var globalCustomerName = document.getElementById("txtCustomerName").value.trim() || "Unknown Customer";
    var globalOrderNum = document.getElementById("txtOrderNum").value.trim() || "01";

    if (!isManual && !selectedCSVPath) { alert("Please select a CSV File or Folder."); return; }
    ...
    try {
        ...
        if (isManual) {
            ...
            var headers = ["front size", "total qty", "half sleeve", "full sleeve"];
            var dataRows = [];
            var sizes = ["18","20","22","24","26","28","30","32","34","36","38","40","42","44","46","48","50","52","54","56","58","60"];
            sizes.forEach(function(sz) {
            ...
            var virtualFileName = globalCustomerName.replace(/[\/\\:*?"<>|]/g, "_") + " - " + globalOrderNum + ".csv";
            mappedJobs.push({ design: { name: "Single_Design" }, file: { name: virtualFileName, path: "" }, rows: dataRows, headers: headers });
            ...
            if (csvFiles.length === 0) { alert("No CSV files found in selected folder."); return; }

            log(">>> Scanning Master Template Design sublayers (Multi-Mode)...");
            var designNames = scanDesignLayers('Front');
            
            if (designNames.length === 0) { alert("No design layers found under 'Front' layer group."); return; }
            log("Found design sublayers: " + designNames.join(", "));

            // Map each design sublayer to a CSV file in the directory
            for (var dIndex = 0; dIndex < designNames.length; dIndex++) {
                var dName = designNames[dIndex];
                var numMatch = dName.match(/\d+/);
                var dNum = numMatch ? parseInt(numMatch[0], 10) : null;
                var prefix = dName.substring(0, 2).trim();

                var matchedFile = null;
                for (var fIndex = 0; fIndex < csvFiles.length; fIndex++) {
                    var f = csvFiles[fIndex];
                    var csvNumMatch = f.match(/\d+/);
                    var csvNum = csvNumMatch ? parseInt(csvNumMatch[0], 10) : null;
                    if (dNum !== null && csvNum !== null && dNum === csvNum) {
                        matchedFile = f;
                        break;
                    }
                }
                if (!matchedFile) {
                    for (var fIndex = 0; fIndex < csvFiles.length; fIndex++) {
                        var f = csvFiles[fIndex];
                        if (f.toLowerCase().indexOf(prefix.toLowerCase()) > -1) {
                            matchedFile = f;
                            break;
                        }
                    }
                ...
                if (matchedFile) {
                    var fPath = selectedCSVPath + "\\" + matchedFile;
                    var csvText = readLocalFile(fPath);
                    ...
                    if (dataRows.length > 0) {
                        mappedJobs.push({ design: { name: dName }, file: { name: matchedFile, path: fPath }, rows: dataRows, headers: headers });
                        totalSteps += dataRows.length * 6;
                        if (doNameNum) totalSteps += dataRows.length;
                    }
                } else {
                    log("⚠️ Skipped Design [" + dName + "] - No matching CSV found.");
                }
            }
        } 
        else {
            ...
            if (dataRows.length > 0) {
                var fileName = selectedCSVPath.split(/[\\\/]/).pop();
                mappedJobs.push({ design: { name: "Single_Design" }, file: { name: fileName, path: selectedCSVPath }, rows: dataRows, headers: headers });
                totalSteps = dataRows.length * 6;
                if (doNameNum) totalSteps += dataRows.length;
            }
        }

        if (mappedJobs.length === 0) { alert("No valid CSV data rows mapped. Check layout names."); return; }
        ...
        for (var jIndex = 0; jIndex < mappedJobs.length; jIndex++) {
            var job = mappedJobs[jIndex];
            var actualOrderCode = job.file.name.replace(/\.[^/.]+$/, "");
            log("\n>>> PROCESSING TEAM: " + (job.design.name === "Single_Design" ? job.file.name : job.design.name));
            
            var cleanDesignName = job.design.name === "Single_Design" ? actualOrderCode : job.design.name;
            cleanDesignName = cleanDesignName.replace(/[\/\\:*?"<>|]/g, "_").trim();

            var designFolder = selectedFolderPath + "\\" + cleanDesignName;
            if (!fso.FolderExists(designFolder)) fso.CreateFolder(designFolder);

            var frontDir = designFolder + "\\Front";
            var backDir = designFolder + "\\Back";
            var sleeveDir = designFolder + "\\Sleeve";
            var nnDir = doNameNum ? designFolder + "\\Name_Number" : null;
            ...
            // In Multi-Team mode, toggle sub-layers visibility in Front/Back layout
            if (isMultiTeam) {
                setDesignVisibility('Front', job.design.name);
                setDesignVisibility('Back', job.design.name);
            }
            ...
            // Merge Half Sleeves
            if (chkMergeHalf) {
                log(">>> Merging Half Sleeves for Design: " + cleanDesignName);
                for (var i = 0; i < job.rows.length; i++) {
                    var row = job.rows[i];
            ...
            // Generate mockups
            if (doMockup) {
                log(">>> Generating Mockups for Design: " + cleanDesignName);
                var mockupsFolder = designFolder + "\\Mockups";
                if (!fso.FolderExists(mockupsFolder)) fso.CreateFolder(mockupsFolder);
                
                var mockupResult = generateMockups(mockupsFolder, actualOrderCode);
...
function saveSizeFromUI() {
    ...
    localStorage.setItem("fivenest_sizes_enterprise", JSON.stringify(sizeDB));
...
function loadDatabase() {
    try {
        var str = localStorage.getItem("fivenest_sizes_enterprise");
        if (str) {
...
function updateUsageDisplay() {
    ...
    if (tag) {
        tag.innerText = "ENTERPRISE V5.1 | Usage: " + currentUsage + "/UNLIMITED";
    }
}
...
async function exportAndSendToFiveNest() {
    try {
        ...
        // Temporary folder for JPG exports
        ...
        for (var pIdx = 0; pIdx < panelDefs.length; pIdx++) {
            ...
            // Strategy 1: Check for tagged shape
